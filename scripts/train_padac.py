...
# Ground truth
# LABELS_PATH = Path('/import/c4dm-04/mary/maestro_labels')
...
def build_dataset(
    ...
    for _, v in folders.items():
        loader = AudioLoader(sources=v, num_channels=1) # specify here whether to use multi-instrument or pitch-only labels
        transform = build_transform()
        dataset = AudioDataset(loader, sample_rate, transform=transform, num_channels=1)
...
class State:
    ...
    pitch_loss: torch.nn.BCEWithLogitsLoss()
...
def load(
    ...
    if resume:
        ...
        print('DAC model successfully loaded from previous checkpoint!')
    ...
    if args['DAC.add_conditioner']:
        assert generator.add_conditioner == True, print("Generator model does not include a Conditioner although args['DAC.add_conditioner'] is True!")
...
@timer()
@torch.no_grad()
def val_loop(batch, state, accel, recon_mode=True):
    ...
    if state.generator.add_conditioner:
        return_dict["pitch/loss"] = state.pitch_loss(out["pitch_class"], batch["pitch_labels"])
...
@timer()
def train_loop(state, batch, accel, lambdas, recon_mode=True):
    ...
    with accel.autocast():
        ...
        pitch_class = out["pitch_class"] if state.generator.add_condtioner else None
    ...
    with accel.autocast():
        ...
        if state.generator.add_conditioner:
            output["pitch/loss"] = state.pitch_loss(pitch_class, batch["pitch_labels"])
...
def checkpoint(state, save_iters, save_path):
    ...
    try:
        ...
        pass
    if state.tracker.is_best("val", "pitch/loss"):
        state.tracker.print(f"Best classifier so far")
...
def validate(state, val_dataloader, accel, recon_mode=True):
    for batch in val_dataloader:
        output = val_loop(batch, state, accel, recon_mode)
...
def train(
    ...
    state = load(args, accel, tracker, save_path, load_weights=False)
    ...
    global train_loop, val_loop, validate, git , checkpoint
    ...
    with tracker.live:
        for tracker.step, batch in enumerate(train_dataloader, start=tracker.step):
            train_loop(state, batch, accel, lambdas, recon_mode)
            ...
            if tracker.step % valid_freq == 0 or last_iter:
                validate(state, val_dataloader, accel, recon_mode)
...
if __name__ == "__main__":
    ...
    with argbind.scope(args):
        with Accelerator() as accel:
            ...
            if args['DAC.add_conditioner']:
                print('Starting training with pitch conditioning in latent space.')
